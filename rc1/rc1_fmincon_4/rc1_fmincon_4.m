...
n = 1;                                      % number of rc links
i_r = zeros(n, 1);
v_c = zeros(n, 1);
flag = 1;

%% compute soc
ind_init = interp1(soc, 1:length(soc), 0.6133, 'nearest');
v_t(1) = ocv(ind_init);
z0 = soc(ind_init);
...
% run fmincon
fun = @(x) opt(x, V_batt_time, I, z, soc, dt, s, v_t, i_r, v_c, ocv, n);
[rc_param, fval] = fmincon(fun,x_0,A,B,Aeq,Beq,lb,ub)
r0 = rc_param(1);                              % [ohm]
    for k = 1:n
        r(k) = rc_param(2*k);                  % [ohm]
        c(k) = rc_param(2*k+1);                % [farad]
    end
