...
V_batt_time_rc = - V_batt_time_rc(i+4) + V_batt_time_rc(i+4:end);
% V_batt_time_rc = V_batt_time_rc(i:end);
I_rc = [0; 10*ones(s-i-3, 1)];
time_rc = time(i+4:end);
...
% x_0 = [1e-3; 1e3; 1e-3; 1e3; 1e-3; 1e3];
% r0 = 3.74e-3;
% r1 = 0.56e-3;
% c1 = 367.98;
% r2 = 0.34e-3;
...
fun = @(x) ga_opt(x, V_batt_time_rc, I_rc, dt, s, i_r, n, i);
x_ga = ga(fun, 2*n);
