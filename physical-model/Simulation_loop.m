...
while time_now < time_max
    ...
    I_e = [I(i)-I_x_neg;ones(n_mesh_sep+3,1)*I(i);I(i)-I_x_pos]';
    ...
    V_x_neg = phis_U_neg(1:end-1)-phis_U_neg(2:end)+I(i)*R_l_neg;
    V_x_neg(1) = V_x_neg(1)+I(i)*R_ct_neg(1);
    I_x_neg = R_x_neg\V_x_neg';
    
    I_ct_neg = [-diff([I(i);I_x_neg]);I_x_neg(end)]'; % charge transfer current per electrode area
    ...
    V_x_pos = phis_U_pos(1:end-1)-phis_U_pos(2:end)+I(i)*R_l_pos;
    V_x_pos(end) = V_x_pos(end)+I(i)*R_ct_pos(end);
    I_x_pos = R_x_pos\V_x_pos';
    
    I_ct_pos = [I_x_pos(1);diff([I_x_pos;I(i)])]'; % charge transfer current per electrode area
    I_loc_pos = I_ct_pos./(Sa_pos*volume_length_pos); % charge transfer current per active area
    %======================================================================
    % record the data
    k = k+1;
    Ah_time(k) = Ah_time(k-1)+I(i)*time_step/3600*0.1; % mAh/cm2
